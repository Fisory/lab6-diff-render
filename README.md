...
【视频1:剪影优化过程——球体经过500步迭代逐渐变形为奶牛轮廓，4个视角同步显示】

【视频2:联合纹理优化——形状与顶点颜色同步收敛，灰球逐渐变成带棕白色的奶牛】
...
### 方法二：WSL2 / Linux（推荐，可直接用预编译 wheel）

```bash
# 建虚拟环境
python3 -m venv lab6_env && source lab6_env/bin/activate

# 安装 PyTorch 2.4.0 CPU 版
pip install torch==2.4.0 torchvision==0.19.0 \
  --index-url https://download.pytorch.org/whl/cpu

# 安装 pytorch3d（Linux 预编译 wheel，Python 3.11/3.12 均有）
...
```bash
# Part 1：剪影优化（500 步，CPU 约 30~60 分钟）
...
【视频3:损失曲线——total / sil / lap / edge / normal 五条曲线随迭代的变化，edge 最高，sil 持续下降】

500 步迭代后，从 20 个视角看，球体的轮廓已经和奶牛剪影基本对齐，躯干、腿部、头部的大致形态都能识别出来。最终总损失降到 0.02 以下，剪影误差约 0.01~0.014。
...
- **学习率调度**：每 150 步学习率减半（$1\text{e-}2 \to 5\text{e-}3 \to 2.5\text{e-}3$），前期快速收敛，后期精细调整。

### Part 2（选做）：联合纹理优化

【视频4:RGB 优化过程——4 个视角同时展示，顶点颜色从灰色（sigmoid(0)=0.5）逐渐拟合奶牛的棕白色纹理】
...
颜色收敛比形状快，100~150 步内颜色就趋近目标了；形状则需要完整的 500 步。两组参数使用不同学习率（顶点位移 `5e-3`，颜色 `1e-1`）放在同一个 Adam 优化器中。
